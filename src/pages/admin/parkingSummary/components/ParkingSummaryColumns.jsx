...
export const parkingSummaryColumns = (navigate) => [
  { name: "Reservation ID", selector: (row) => row?.reservationId },
  { name: "Building Name", selector: (row) => row?.buildingName },
  { name: "Phone Number", selector: (row) => row?.phoneNumber },
  { name: "Vehicle Info", selector: (row) => row?.vehicleInfo },
  { name: "Reservation Date", selector: (row) => row?.reservationDate },
  { name: "Payment Status", selector: (row) => row?.paymentStatus },
  {
    name: "Slip",
    selector: (row) => (
      <div className="cursor-pointer" onClick={() => navigate(`/admin/view-slip/${row?.id}`)}>
        <SlipIcon />
      </div>
    ),
  },
